...
static int32_t sdmmc_execute_command(LPC_SDMMC_Type *pSDMMC, uint32_t cmd, uint32_t arg, uint32_t wait_status)
{
...
static void prv_process_csd(LPC_SDMMC_Type *pSDMMC)
{
    int32_t status = 0;
    int32_t c_size = 0;
...
static int32_t prv_set_trans_state(LPC_SDMMC_Type *pSDMMC)
{
    uint32_t status;
...
static int32_t prv_set_card_params(LPC_SDMMC_Type *pSDMMC)
{
    int32_t status;
...
int32_t Chip_SDMMC_GetState(LPC_SDMMC_Type *pSDMMC)
{
    uint32_t status;
...
uint32_t Chip_SDMMC_Acquire(LPC_SDMMC_Type *pSDMMC, mci_card_struct *pcardinfo)
{
    int32_t status;
    int32_t tries = 0;
...
uint64_t Chip_SDMMC_GetDeviceSize(LPC_SDMMC_Type *pSDMMC)
{
    return g_card_info->card_info.device_size;
}

/* Get the number of blocks in SD/MMC card (after enumeration) */
int32_t Chip_SDMMC_GetDeviceBlocks(LPC_SDMMC_Type *pSDMMC)
{
    return g_card_info->card_info.blocknr;
}

/* Performs the read of data from the SD/MMC card */
int32_t Chip_SDMMC_ReadBlocks(LPC_SDMMC_Type *pSDMMC, void *buffer, int32_t start_block, int32_t num_blocks)
{
...
int32_t Chip_SDMMC_WriteBlocks(LPC_SDMMC_Type *pSDMMC, void *buffer, int32_t start_block, int32_t num_blocks)
{
